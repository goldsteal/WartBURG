/* wartburg_theme.c - BURG theme parser ported into WartBURG.
 *
 * Verbatim port of bean's BURG generic tree (lib/tree.c) + UI tree / theme
 * parser (lib/uitree.c). Only platform glue adapted to GRUB 2.15:
 *   - BURG's GRUB_EXPORT(...) markers dropped (single-module, no cross-module export)
 *   - grub_file_open() takes a file-type arg in modern GRUB
...
/* ===== generic intrusive tree (BURG lib/tree.c) ===== */
...
/* ===== UI node tree + theme parser (BURG lib/uitree.c) ===== */
...
grub_uitree_load_file (grub_uitree_t root, const char *name, int flags)
{
  ...
  file = grub_file_open (name, GRUB_FILE_TYPE_THEME);
  if (! file)
    return 0;
